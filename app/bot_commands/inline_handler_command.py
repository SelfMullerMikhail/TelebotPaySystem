...
class Inline:
    ...
    def get_goods_buttons(self, goods_id, callback_data_info, price, title,
        ...
        keyboard_btn = (types.InlineKeyboardButton(
            text=APPROVE_TEXT, callback_data=callback_data_info))
        keyboard_mark.add(keyboard_btn)
        main_text = f"""{title} - {price} ₽"""

        btn = (types.InlineQueryResultArticle(
            id=goods_id, title=main_text,
            input_message_content=types.InputTextMessageContent(
                message_text=main_text), reply_markup=keyboard_mark))
        return btn
