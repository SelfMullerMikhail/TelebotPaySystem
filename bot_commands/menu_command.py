...
dotenv_path = os.path.join(os.path.dirname(os.path.dirname(os.path.
                                        abspath(__file__))), 'env_file.env')
...
class Menu:
    ...
    def get_category_keyboard(self, buttons_categorys:list) -> (telebot.types.
                                                        InlineKeyboardMarkup):
        keyboard = telebot.types.InlineKeyboardMarkup()
        for butn_name, id in buttons_categorys:
            button = telebot.types.InlineKeyboardButton(text=butn_name,
                                                callback_data=id)
            keyboard.add(button)
        return keyboard
        
    def get_category(self):
        self.goodsCategory = ApiAqsiClasses(self.__STRIPE_SECRET_KEY).get_goodsCategory()
